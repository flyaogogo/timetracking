...
public interface DashboardMapper {
     ...
     * 获取用户在参与项目上花费的总工时
     */
    @Select("SELECT COALESCE(SUM(te.duration), 0) FROM (SELECT DISTINCT id, duration FROM time_entries WHERE user_id = #{userId}) te INNER JOIN time_entries te_full ON te.id = te_full.id INNER JOIN projects p ON te_full.project_id = p.id LEFT JOIN project_members pm ON p.id = pm.project_id WHERE (pm.user_id = #{userId} OR p.manager_id = #{userId})")
